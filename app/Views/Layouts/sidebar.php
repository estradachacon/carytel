...
#layoutSidenav_nav {
    ...
    overflow: hidden;          /* no clipping — collapse panels + flyouts must escape */
...
body.sb-mini #layoutSidenav_nav { width: 64px; }
...
body.sb-mini #layoutSidenav_content { margin-left: 64px !important; }
...
.sidebar-brand img.logo { height: 36px; border-radius: 8px; object-fit: contain; flex-shrink: 0; }
...
body.sb-mini .sidebar-brand-text { opacity: 0; pointer-events: none; }
/* Hide entire brand header in mini mode — menu starts right below navbar */
body.sb-mini .sidebar-brand { display: none !important; }

.sidebar-brand-name {
    font-family: 'Plus Jakarta Sans', 'Inter', sans-serif !important;
    font-size: .9rem; font-weight: 800;
    color: white; white-space: nowrap; line-height: 1.2;
}
.sidebar-branch { font-size: .68rem; color: rgba(255,255,255,.45); white-space: nowrap; margin-top: 1px; }
...
.sb-sidenav-menu {
    ...
    overflow-x: hidden;
    padding: .4rem 0 1rem;
    background: #0f172a;        /* own bg so it covers the nav's visible overflow */
    position: relative;
    z-index: 1;
}
.sb-sidenav-menu::-webkit-scrollbar { width: 3px; }
.sb-sidenav-menu::-webkit-scrollbar-thumb { background: rgba(255,255,255,.1); border-radius: 3px; }
...
body.sb-mini .sb-sidenav-menu-heading { display: none !important; }
...
.sb-sidenav-menu > .nav > .nav-link.active .sb-nav-link-icon { opacity: 1 !important; }

/* ─── LABEL & ARROW ────────────────────────────────────────────────── */
.nav-link-label { flex: 1; overflow: hidden; text-overflow: ellipsis; }
...
body.sb-mini .sb-sidenav-collapse-arrow { display: none !important; }

/* ─── HIDE COLLAPSES IN MINI ───────────────────────────────────────── */
body.sb-mini .sb-sidenav-menu .collapse { display: none !important; }
...
.sb-sidenav-menu-nested .sb-sidenav-menu-nested .nav-link { padding-left: 2.8rem !important; }
.sb-sidenav-menu-nested .sb-sidenav-menu-heading { padding: .8rem .75rem .25rem 2rem !important; font-size: .58rem !important; }
...
.sb-flyout {
    ...
    z-index: 9990;              /* above everything — lives on body, global stacking */
    display: none;
    padding: 6px 0 8px;
}
.sb-flyout.sb-flyout-on { display: block; }
body:not(.sb-mini) .sb-flyout { display: none !important; }
...
.sb-flyout .nav-link:hover { color: rgba(255,255,255,.9) !important; background: rgba(255,255,255,.07) !important; }
.sb-flyout .nav-link.active { color: #38bdf8 !important; background: rgba(26,86,219,.18) !important; font-weight: 600 !important; }
...
.close-mobile-nav:hover { color: white; }
...
@media (max-width: 768px) {
    ...
    #layoutSidenav_nav.sb-open { left: 0 !important; box-shadow: 8px 0 40px rgba(0,0,0,.55) !important; }
    body.sb-mini #layoutSidenav_nav { width: 240px !important; }
...
<div id="layoutSidenav_nav">
    ...
    <nav class="sb-sidenav accordion" id="sidenavAccordion">
        ...
        <div class="sb-sidenav-menu">
            <div class="nav flex-column">
                ...
                <a class="nav-link collapsed" href="#"
                   data-bs-toggle="collapse" data-bs-target="#sales"
                   aria-expanded="false" aria-controls="sales"
                   data-flyout-id="sales">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-box-open"></i></div>
                    <span class="nav-link-label">Paquetería</span>
                    <div class="sb-sidenav-collapse-arrow"><i class="fa-solid fa-angle-down"></i></div>
                </a>
                <div class="collapse" id="sales" data-bs-parent="#sidenavAccordion">
                    <nav class="sb-sidenav-menu-nested nav flex-column">
                ...
                <a class="nav-link collapsed" href="#"
                   data-bs-toggle="collapse" data-bs-target="#treasury"
                   aria-expanded="false" aria-controls="treasury"
                   data-flyout-id="treasury">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-hand-holding-dollar"></i></div>
                    <span class="nav-link-label">Remuneraciones</span>
                    <div class="sb-sidenav-collapse-arrow"><i class="fa-solid fa-angle-down"></i></div>
                </a>
                <div class="collapse" id="treasury" data-bs-parent="#sidenavAccordion">
                    <nav class="sb-sidenav-menu-nested nav flex-column">
                ...
                <a class="nav-link" href="/sellers" data-label="Vendedores">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-address-book"></i></div>
                    <span class="nav-link-label">Vendedores</span>
                </a>
                <?php endif; ?>

                <!-- SOLICITUDES -->
                <?php if (tienePermiso('ver_solicitudes')): ?>
                <a class="nav-link" href="<?= base_url('solicitudes') ?>" data-label="Solicitudes">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-rotate-left"></i></div>
                    <span class="nav-link-label">Solicitudes de Reversión</span>
                </a>
                <?php endif; ?>
                ...
                <a class="nav-link collapsed" href="#"
                   data-bs-toggle="collapse" data-bs-target="#settledpoint"
                   aria-expanded="false" aria-controls="settledpoint"
                   data-flyout-id="settledpoint">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-map-location-dot"></i></div>
                    <span class="nav-link-label">Puntos fijos y Rutas</span>
                    <div class="sb-sidenav-collapse-arrow"><i class="fa-solid fa-angle-down"></i></div>
                </a>
                <div class="collapse" id="settledpoint" data-bs-parent="#sidenavAccordion">
                    <nav class="sb-sidenav-menu-nested nav flex-column">
    ...
    <!-- ── FLYOUT PANELS (position:fixed, visible outside overflow:hidden) ── -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('#layoutSidenav_nav .nav-link').forEach(function (link) {
        ...
        if (currentPath === normalized) {
            ...
            document.querySelectorAll('.sb-flyout .nav-link[href="' + href + '"]')
                .forEach(function (fl) { fl.classList.add('active'); });
        }
    });
    ...
    function hideFlyouts() {
        document.querySelectorAll('.sb-flyout.sb-flyout-on')
            .forEach(function (f) { f.classList.remove('sb-flyout-on'); });
    }
    ...
    document.querySelectorAll('[data-flyout-id]').forEach(function (trigger) {
        ...
        // Hover: show flyout
        trigger.addEventListener('mouseenter', function () { positionAndShow(trigger, flyout); });
        trigger.addEventListener('mouseleave', function () { hideTimer = setTimeout(hideFlyouts, 150); });
        flyout.addEventListener('mouseenter', function () { clearTimeout(hideTimer); });
        flyout.addEventListener('mouseleave', function () { hideTimer = setTimeout(hideFlyouts, 150); });
    ...
    document.querySelectorAll('.sb-sidenav-menu > .nav > .nav-link[data-label]').forEach(function (link) {
        ...
        link.addEventListener('mouseleave', function () { tip.style.display = 'none'; });
    });
